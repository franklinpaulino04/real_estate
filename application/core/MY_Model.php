...
class MY_Model extends CI_Model {
    ...
    /**
     * Generate datatables
     * Ignited Datatables is a the new version of the Datatables CI Library ver. 2.0. It was renamed to ignitedtables by Nelson Cabrera to avoid conflict with legacy code.
     * TODO: This will be replacing all instances of the previous Datatable library ver. 0.6
     * Added by Nelson Cabrera on 03/31/2017
     * @param string
     * @param array
     * @return array
     * @author Nelson Cabrera
     */
    public function ignitedtables($columns = FALSE, $where = FALSE, $companyId = TRUE, $add_columns = FALSE, $where_in = FALSE, $view_name = FALSE, $no_ids = FALSE, $id_name = FALSE)
    {
        if($companyId === TRUE)
        {
            $this->ignitedtables->where('companyId', $this->companyId);
        }

        elseif($where_in !== FALSE)
        {

            $id_name = ($id_name != FALSE)? $id_name : $this->primary_key;
            // always convert to array
            is_array($where_in) || $where_in = array($where_in);
            $this->ignitedtables->where_in($id_name, $where_in);
        }

        if($where !== FALSE)
        {
            $this->ignitedtables->where($where);
        }
...
//        $view_name = ($view_name == FALSE)? $this->view_name : $view_name;


//        $this->ignitedtables->select($columns, FALSE)->from("(SELECT @p1:=$this->companyId p) parm, $view_name")->add_column('accion', '', '');
        $view_name = ($view_name == FALSE)? "(SELECT @p1:=$this->companyId p) parm, $this->view_name" : $view_name;

        $this->ignitedtables->select($columns, FALSE)->from($view_name)->add_column('accion', '', '');

        if($add_columns !== FALSE)
        {
            is_array($add_columns) || $add_columns = array($add_columns);

            foreach($add_columns AS $add_column)
            {
                $this->ignitedtables->add_column($add_column, '', '');
            }
        }

        return $this->ignitedtables->generate();
    }
     ...
     * @author Nelson Cabrera
     */
    public function datatables($columns = FALSE, $where = FALSE, $companyId = TRUE, $add_columns = FALSE, $where_in = FALSE)
    {
        if($companyId === TRUE)
        {
            $this->datatables->where('companyId', $this->companyId);
        }
        ...
        $this->datatables->select($columns, FALSE)->from("(SELECT @p1:=$this->companyId p) parm, $this->view_name")->add_column('accion', '', '');
        ...
        if($add_columns !== FALSE)
        {
            ...
            foreach($add_columns AS $add_column)
            {
                $this->datatables->add_column($add_column, '', '');
            }
        }
    ...
    /**
     * Generate datatables extends
     * @param string
     * @param array
     * @return array
     * @author Nelson Cabrera
     */
    public function datatables_extends($columns = FALSE, $where = FALSE, $companyId = TRUE, $add_columns = FALSE, $where_in = FALSE)
    {
        if($companyId === TRUE)
        {
            $this->datatables->where('companyId', $this->companyId);
        ...
        $this->datatables->select($columns, FALSE)->from("$this->view_name")->add_column('accion', '', '');
