...
class Cp_abouts extends CP_Controller
{
    public $namespace;
    public $columns;
    public $title;
...
//        $this->load->module('com_accounts/controllers/com_accounts');
    ...
    {
        $data['content']               = 'cp_home/cp_home_view';
    ...
    {
        $result = $this->cp_home_model->ignitedtables($this->columns);
    ...
    {
        $data                           = array();
        $data['row']                    = $this->cp_home_model->get($annulmentId);
    ...
    public function insert()
    {
        ...
        else
        {
            ...
            if($this->cp_home_model->save($data))
            {
    ...
    public function update($annulmentId)
    {
        ...
        else
        {
            ...
            if($this->cp_home_model->save($data, $annulmentId))
            {
                echo json_encode(array('result' => 1));
            }
        }
    ...
    {
        $result['result'] = ($this->cp_home_model->save( array('hidden' => 1), $annulmentId) == TRUE )? 1 : 0;
    ...
    private function annulment_type_check($annulment_type = FALSE, $annulmentId = FALSE)
    {
        ...
        return ($this->cp_home_model->in_table_by($where) > 0)? FALSE : TRUE;
    }
